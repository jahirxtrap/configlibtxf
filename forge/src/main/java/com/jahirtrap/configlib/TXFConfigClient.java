...
public class TXFConfigClient extends TXFConfig {
    ...
    public static String getModName(String modId) {
        return ModList.getModContainerById(modId).map(c -> c.getModInfo().getDisplayName()).orElse(modId);
    }
                    ...
                    if (info.function != null) {
                        ...
                        if (info.field.getType() == List.class && info.listExpanded) {
                            ...
                            for (int idx = 0; idx < listValues.size(); idx++) {
                                final int index = idx;
                                EditBox listField = new FilteredField(font, width - 185, 0, 150, 20, Component.empty());
                                listField.setMaxLength(info.width);
                                listField.setValue(listValues.get(index).toString());
                                if (listField instanceof FilteredField ff) {
                                    if (e.isColor()) ff.setCharFilter(c -> HEXADECIMAL_ONLY.matcher(c).matches());
                                    else if (e.idMode() >= 0)
                                        ff.setCharFilter(c -> IDENTIFIER_ONLY.matcher(c).matches());
                                }
