...
public class TXFConfigClient extends TXFConfig {
        ...
        public void updateButtons() {
            if (this.list != null) {
                for (ButtonEntry entry : this.list.children()) {
                    if (entry.info == null || entry.buttons == null || entry.buttons.size() <= 1) continue;
            ...
            for (EntryInfo info : entries) {
                if (info.modid.equals(modid) && (info.tab == null || info.tab == tabManager.getCurrentTab())) {
                    ...
                    if (info.function != null) {
                        ...
                        Button listToggleButton = null;
                        if (info.field.getType() == List.class) {
                            var listValues = (List<?>) info.value;
                            String arrow = info.listExpanded ? "▼" : "▶";
                            listToggleButton = Button.builder(Component.literal(arrow + " " + listValues.size()).withStyle(ChatFormatting.GOLD), (button -> {
                                info.listExpanded = !info.listExpanded;
                                list.clear();
                                fillList();
                            })).bounds(width - 185, 0, 150, 20).build();
                        ...
                        if (info.field.getType() == List.class && info.listExpanded) {
                            ...
                            for (int idx = 0; idx < listValues.size(); idx++) {
                                ...
                                AbstractWidget rowActionButton;
                                if (countLocked) {
                                    Button resetElement = TextAndImageButton.builder(Component.empty(), new ResourceLocation(MODID, "textures/gui/sprites/icon/reset.png"), (button -> {
                                        var currentList = new ArrayList<>((List<Object>) info.value);
                                    ...
                                    resetElement.setWidth(20);
                                    resetElement.setPosition(width - 205 + 150 + 25, 0);
                                    resetElement.active = !locked && index < ((List<?>) info.defaultValue).size()
                                            && !Objects.equals(listValues.get(index).toString(), ((List<?>) info.defaultValue).get(index).toString());
                                    itemReset[0] = resetElement;
                                    rowActionButton = resetElement;
                                } else {
                                    ...
                                    removeButton.active = !locked && listValues.size() > minCount;
                                    rowActionButton = removeButton;
                                }
    ...
    public static class ConfigListWidget extends ContainerObjectSelectionList<ButtonEntry> {
        public ConfigListWidget(Minecraft client, int i, int j, int k, int l, int m) {
