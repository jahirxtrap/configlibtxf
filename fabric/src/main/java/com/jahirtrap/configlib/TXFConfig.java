...
public abstract class TXFConfig {
    ...
    private static void initClient(String modid, Field field, EntryInfo info) {
        ...
        if (e != null) {
            if (!e.name().isEmpty()) info.name = new TextComponent(e.name());
            if (info.dataType == int.class) textField(info, Integer::parseInt, INTEGER_ONLY, (int) e.min(), (int) e.max(), true);
                ...
                Function<Object, Component> func = value -> {
                    ...
                    return I18n.exists(translationKey) ? new TranslatableComponent(translationKey) : new TextComponent(info.toFormattedName());
                };
                info.function = new AbstractMap.SimpleEntry<Button.OnPress, Function<Object, Component>>(button -> {
    ...
    public static class ConfigScreen extends Screen {
        protected ConfigScreen(Screen parent, String modid) {
            super(I18n.exists(modid + ".config.title") ? new TranslatableComponent(modid + ".config.title") : new TextComponent(getModName(modid)));
        ...
        public void fillList() {
            for (EntryInfo info : entries) {
                if (info.modid.equals(modid)) {
                    ...
                    if (info.name == null && !I18n.exists(translationPrefix + info.field.getName())) name = new TextComponent(info.toFormattedName());
