...
public abstract class TXFConfig {
    ...
    private static void textField(EntryInfo info, Function<String,Number> f, Pattern pattern, double min, double max, boolean cast) {
        ...
        info.function = (BiFunction<EditBox, Button, Predicate<String>>) (t, b) -> s -> {
            ...
            if (!(isNumber && s.isEmpty()) && !s.equals("-") && !s.equals(".")) {
                ...
                info.error = inLimits? null : Component.translatable(value.doubleValue() < min ?
            ...
            if (info.field.getAnnotation(Entry.class).isColor()) {
                ...
                try { info.actionButton.setMessage(Component.translatable("⬛").setStyle(Style.EMPTY.withColor(Color.decode(info.tempValue).getRGB())));
    ...
    public static class ConfigScreen extends Screen {
        ...
        public void init() {
            ...
            this.addRenderableWidget(new SpriteIconButton(this.width / 2 - 182, this.height - 27, 20, 20, Component.empty(), button -> Util.getPlatform().openFile(FabricLoader.getInstance().getConfigDir().resolve(modid + ".json").toFile()), new ResourceLocation("configlibtxf","textures/gui/sprites/icon/editor.png"), 12, 12));
                ...
                if (info.modid.equals(modid)) {
                    ...
                    if (info.function != null) {
                        ...
                        if (info.function instanceof Map.Entry) { // Enums & booleans
                            ...
                            widget = new SliderWidget(width - 185, 0, 150, 20, Component.translatable(info.tempValue), (Double.parseDouble(info.tempValue) - e.min()) / (e.max() - e.min()), info);
                        ...
                        if (info.field.getType() == List.class) {
                            cycleButton = new Button(width - 185, 0, 20, 20, Component.translatable(String.valueOf(info.listIndex)).withStyle(ChatFormatting.GOLD), (button -> {
                        ...
                        if (e.isColor()) {
                            Button colorButton = new Button(width - 185, 0, 20, 20, Component.translatable("⬛"),
                                    button -> new Thread(() -> {
                            ...
                            try { colorButton.setMessage(Component.translatable("⬛").setStyle(Style.EMPTY.withColor(Color.decode(info.tempValue).getRGB())));
                            } catch (Exception ignored) {}
                            ...
                            Button explorerButton = new SpriteIconButton(width - 185, 0, 20, 20, Component.empty(),
                                    button -> new Thread(() -> {
